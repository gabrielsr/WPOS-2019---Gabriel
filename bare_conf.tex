...
\title{GoalD: A Goal-Driven Deployment Framework for Dynamic and Heterogeneous Computing Environments}
...
Departamento de CiÃªncia da ComputaÃ§Ã£o \\
 Universidade de BrasÃ­lia (UnB)\\
  Campus UniversitÃ¡rio Darcy Ribeiro - Asa Norte \\
  BrasÃ­lia, DF, Brazil 70910-900 \\
...
\label{fig:mapek}
\end{figure}
 % We present GoalD's deployment metamodel in the following section (Section~\ref{sec:metamodel}) as the underlying structure that defines major conceptual elements of GoalD's autonomous deployment.
...
% \todo[inline]{Two contributiuons: not deploy a system if the required SLA can not be achieved. 
% If more then one alternative is available, choose the best alternative}
...
%\begin{figure}[!t]
%\centering
%\includegraphics[width=2.5in]{transformations}
